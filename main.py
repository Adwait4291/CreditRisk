...
def load_data(file_path):
    """
    Loads data from a CSV file, trying different encodings if UTF-8 fails.
    ...
    for encoding in encodings_to_try:
        try:
            ...
            logging.info(f"Successfully loaded data from {os.path.basename(file_path)} using encoding '{encoding}'. Shape: {df.shape}")
            break # Exit loop if loading succeeds
        except UnicodeDecodeError:
            logging.warning(f"Failed to load {os.path.basename(file_path)} with encoding '{encoding}'. Trying next...")
        except Exception as e:
            logging.error(f"Error loading data from {file_path} with encoding '{encoding}': {e}", exc_info=True)
            # Break on non-encoding errors
            return None 
            
    if df is None:
         logging.error(f"Failed to load data from {file_path} after trying encodings: {encodings_to_try}")
         return None
...
def handle_missing_values_df2(df, placeholder=MISSING_VALUE_PLACEHOLDER, drop_thresh=10000):
    ...
    # Important: Check if placeholder exists before using isin if it might not be present
    if placeholder in pd.unique(df.values.ravel('K')):
        ...
        rows_removed_count = 0 # Placeholder wasn't found
...
def preprocess_data():
    ...
    # *** Ensure config.CASE_STUDY1_FILE and config.CASE_STUDY2_FILE point to the CSV files ***
